...
const CaretakerCard = ({ caretaker }) => {
    const { photo, name, rating, available, title, specialties, description, hourlyRate } = caretaker;
    return (
        <div>
            <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm hover:shadow-lg transition-all group">
                <div className="relative mb-4">
                    ...
                    <div className="absolute top-2 right-2 rounded-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm px-2 py-1 flex items-center gap-1 text-xs font-bold text-slate-900 dark:text-white">
                        <MdStar className="text-yellow-500 text-[14px]" /> {rating}
                    </div>
                </div>

                <div className="flex items-center justify-between mb-1">
                    <h4 className="font-display font-bold text-slate-900 dark:text-slate-100">{name}</h4>
                    {available && (
                ...
                <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4">
                    ...
                    <Link href={`/caregivers/${caretaker._id}`} className="rounded-lg bg-slate-100 dark:bg-slate-800 px-3 py-1.5 text-xs font-bold hover:bg-primary hover:text-white transition-colors group-hover:bg-primary group-hover:text-white">
                        View Details
                    </Link>
                </div>
            </div>
